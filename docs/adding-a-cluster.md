...
* Create `Servers` for each node using UUIDs taken from Terraform config.
* Generate certificates for MC `kubeconfig` and `talosconfig` at `/hack/pki`.
